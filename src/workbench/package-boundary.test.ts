...
const allowedHostImports = new Set([
  '@forgeax/workbench-host/browser',
  '@forgeax/workbench-host/contracts',
  '@forgeax/workbench-host/react',
]);
...
describe('workbench host dependency boundary', () => {
  test('pins shared host and extension platform releases exactly', () => {
    const manifest = JSON.parse(readFileSync(join(packageRoot, 'package.json'), 'utf8'));
    expect(manifest.dependencies?.['@forgeax/workbench-host']).toBe('0.1.0');
    expect(manifest.dependencies?.['@forgeax/extension-platform']).toBe('0.0.2');
  });

  test('imports only browser, React, and contracts subpaths', () => {
