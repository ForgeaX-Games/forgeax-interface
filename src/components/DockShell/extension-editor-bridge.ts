...
function parseRequest(value: unknown): EditorAssetImportSourceRequest | null {
  if (!isRecord(value) || value.type !== 'extension:editor-asset-import') return null;
 ...
 * Page iframes. It owns only the message envelope; the injected callback
...
export function attachExtensionEditorAssetImportBridge(
  ...
  const onMessage = (event: MessageEvent): void => {
    ...
    if (!request) {
      if (isRecord(event.data) && event.data.type === 'extension:editor-asset-import') {
        source.postMessage({
