// packages/interface/src/core/plugins/foundation-panels.test.ts
import { describe, expect, it } from 'bun:test';
import type React from 'react';
import { createAppHost } from '../app-shell/host';
import { foundationPanelsPlugin } from './foundation-panels';
...
describe('foundation-panels', () => {
  it('contributePanels merges patch into host.panels; cleanup removes it', () => {
    ...
    const ctx: any = {
      ...
      contributePanels(patch: any) {
        // proxied by foundationPanelsPlugin's setup — but for the test we
        // use foundationPanelsPlugin's exposed helper directly.
        return foundationPanelsPlugin.contributePanels(host, patch);
      },
    };
    void foundationPanelsPlugin.setup!(ctx);
  ...
  it('cleanup is idempotent — double-call does not restore twice', () => {
    ...
    const off = foundationPanelsPlugin.contributePanels(host, {
