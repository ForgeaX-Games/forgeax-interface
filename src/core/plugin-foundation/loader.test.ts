...
function makeCtx(caps: ReturnType<typeof createCapabilityRegistry<Cap>>): (m: PluginManifest<Cap, Ctx>) => Ctx {
  return () => ({
...
describe('PluginLoader', () => {
  it('activates plugins whose requires are satisfied; skips others until deps arrive', async () => {
    ...
    const loader = createPluginLoader<Ctx, Cap>({ capabilities: caps, contextFactory: makeCtx(caps), devMode: false });
    const seq: string[] = [];
    const p1: PluginManifest<Cap, Ctx> = {
      id: 'p1', version: '1', provides: ['a'],
      setup: (ctx) => { ctx.host.extend('a'); seq.push('p1'); return () => { seq.push('p1-cleanup'); }; },
    };
    const p2: PluginManifest<Cap, Ctx> = {
  ...
  it('setup exception is reported via onError and does not activate the plugin', async () => {
    const caps = createCapabilityRegistry<Cap>();
    const errs: string[] = [];
    const loader = createPluginLoader<Ctx, Cap>({
