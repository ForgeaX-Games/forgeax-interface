// packages/interface/src/core/plugin-foundation/commands.ts
import type { Cleanup } from './types';
import { PluginConflictError } from './errors';
...
export function createCommandsRegistry(): CommandsRegistry {
  ...
  return {
    register(cmd) {
      ...
      for (const l of Array.from(didRegisterListeners)) {
        try { l(cmd.id); } catch (err) {
          console.error('[plugin-foundation] onDidRegister listener threw', err);
        }
      }
    ...
    async execute<R = unknown>(id: string, args?: unknown): Promise<R> {
      ...
      if (!cmd) throw new Error(`[plugin-foundation] command "${id}" not found`);
      if (cmd.when && !cmd.when()) throw new Error(`[plugin-foundation] command "${id}" blocked by "when" predicate`);
