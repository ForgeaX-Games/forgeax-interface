// packages/interface/src/core/plugin-foundation/loader.ts
//
// Copied from arrival's plugin-foundation/loader.ts. See spec §4 for the
// pending / capability-driven-reactivation semantics.
import type { CapabilityRegistry } from './capabilities';
import { PluginSetupError } from './errors';
import type { Cleanup, PluginManifest } from './types';

const PENDING_WARN_DELAY_MS = 30_000;

export interface PluginLoaderOptions<Ctx, C extends string> {
  readonly capabilities: CapabilityRegistry<C>;
  readonly contextFactory: (manifest: PluginManifest<C, Ctx>) => Ctx;
  readonly onError?: (err: PluginSetupError, manifest: PluginManifest<C, Ctx>, phase: 'setup' | 'cleanup') => void;
  readonly devMode?: boolean;
}

export interface PluginLoader<Ctx, C extends string> {
  load(manifests: ReadonlyArray<PluginManifest<C, Ctx>>): Promise<void>;
  unload(): Promise<void>;
  getPending(): ReadonlyArray<PluginManifest<C, Ctx>>;
  getActive(): ReadonlyArray<PluginManifest<C, Ctx>>;
  flush(): Promise<void>;
}

interface ActiveRecord<Ctx, C extends string> {
  readonly manifest: PluginManifest<C, Ctx>;
  readonly cleanup: Cleanup | undefined;
  readonly order: number;
}

export function createPluginLoader<Ctx, C extends string>(
  opts: PluginLoaderOptions<Ctx, C>,
): PluginLoader<Ctx, C> {
  const { capabilities, contextFactory, onError, devMode = false } = opts;
  const pending = new Map<string, PluginManifest<C, Ctx>>();
  ...
  function enqueue(work: () => Promise<void>): Promise<void> {
    const next = opChain.then(work).catch((err) => {
      console.error('[plugin-foundation] internal opChain error (should not happen — onError catches setup/cleanup)', err);
    });
    opChain = next;
    return next;
  }

  function requiresSatisfied(m: PluginManifest<C, Ctx>): boolean {
    if (!m.requires || m.requires.length === 0) return true;
    for (const r of m.requires) if (!capabilities.has(r)) return false;
    return true;
  }

  function firstReadyPending(): PluginManifest<C, Ctx> | undefined {
    let best: PluginManifest<C, Ctx> | undefined;
  ...
  async function runCleanup(rec: ActiveRecord<Ctx, C>): Promise<void> {
    ...
    try { await rec.cleanup(); }
      ...
      const err = new PluginSetupError({ pluginId: rec.manifest.id, phase: 'cleanup', cause });
      onError?.(err, rec.manifest, 'cleanup') ?? console.error(err);
    }
  }

  async function activateOne(m: PluginManifest<C, Ctx>): Promise<void> {
    ...
    try {
      ...
      const err = new PluginSetupError({ pluginId: m.id, phase: 'setup', cause });
    ...
    for (const rec of victims.slice().sort((a, b) => b.order - a.order)) {
      ...
      if (devMode) {
        ...
        const timer = setTimeout(() => {
          ...
          console.warn(`[plugin-foundation] plugin "${rec.manifest.id}" pending >${PENDING_WARN_DELAY_MS}ms; missing: ${JSON.stringify(missing)}`);
  ...
  return {
      ...
      for (const m of manifests) {
        ...
        if (devMode) {
          const timer = setTimeout(() => {
            ...
            console.warn(`[plugin-foundation] plugin "${m.id}" pending >${PENDING_WARN_DELAY_MS}ms; missing: ${JSON.stringify(missing)}`);
    ...
    getActive() {
      return activeOrder
        ...
        .filter((m): m is PluginManifest<C, Ctx> => m !== undefined);
    },
