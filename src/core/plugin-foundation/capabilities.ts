// packages/interface/src/core/plugin-foundation/capabilities.ts
//
// Observable capability set. Copied from arrival's plugin-foundation/
// capabilities.ts.
  ...
  return {
    ...
    snapshot() {
      ...
      const reject = (op: string) => () => {
        throw new TypeError(`[plugin-foundation] snapshot() is read-only; ${op}() rejected`);
      };
