// packages/interface/src/core/extension-foundation/bus.ts
//
// Synchronous typed pub/sub with middleware chain. Copied from arrival's
// extension-foundation/bus.ts — see docs/comm-mechanism-analysis-2026-07-09.md
// §3.4.
import type {
  BusEvent,
  EventMap,
...
export class EventBus<E extends EventMap> {
  ...
  constructor(opts: EventBusOptions<E> = {}) {
    this.onListenerError =
      ...
      ((err, topic, payload) =>
        console.error(
          `[extension-foundation] EventBus listener for "${String(topic)}" threw`,
          err,
          { payload },
        ));
  }
  ...
  private assertAlive(method: 'on' | 'use'): void {
    if (this.destroyed) {
      throw new Error(`[extension-foundation] EventBus.${method}() called on a destroyed bus`);
    }
  }
}
